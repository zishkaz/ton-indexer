...
class IndexWorker():
    def __init__(self, loop, ls_index, use_ext_method=False, cdll_path=None, keystore=None):
        with open(settings.liteserver_config, 'r') as f:
            tonlib_config = json.loads(f.read())
        
        keystore = keystore or f'/tmp/ton_keystore_{ls_index}'
        ...
        loop.run_until_complete(self.init_worker())
    
    async def init_worker(self):
        await self.client.init()
        mc_info = await self.client.get_masterchain_info()
        logger.info(f"Worker got masterchain block: {mc_info['last']}")
...
async def _get_block_v2(mc_seqno_list):
    async with SessionMaker() as session:
        existing_seqnos = await get_existing_seqnos_from_list(session, mc_seqno_list)
    seqnos_to_process = [seqno for seqno in mc_seqno_list if seqno not in existing_seqnos]
    results = await asyncio.gather(*[index_worker.get_raw_info(seqno) for seqno in seqnos_to_process], return_exceptions=True)

    output = []
    for r in results:
        try:
            blocks, headers, transactions = r
            await insert_by_seqno_core(engine, blocks, headers, transactions)
            output.append(None)
        except:
            output.append(r)
    return seqnos_to_process, output
